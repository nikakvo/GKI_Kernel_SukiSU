...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ath9k", action="store_true",
                        help="Build ath9k_htc + ath9k_common + ath9k_hw + ath as "
                             ...
                             "over OTG. CFG80211/MAC80211 are enabled as =m ONLY so "
                             "the driver can link; nothing wireless goes into the "
                             "Image and the built cfg80211.ko/mac80211.ko are "
                             "deliberately NOT shipped - the device loads Qualcomm's "
                             "from /vendor/lib/modules. Post-build the module CRCs "
                             "are checked against that vendor stack.")
    ...
    parser.add_argument("--extra-net", action="store_true",
                        help="Enable the optional networking config set: IPv6 NAT "
                             ...
                             "sub-options needed to actually use it (notably NFT_COMPAT, "
                             "which iptables-nft in Debian/Kali chroots relies on), the "
                             "four ipset hash types stock GKI omits, plus NET_ACT_CONNMARK, "
                             "INET_RAW_DIAG and CIFS_XATTR. All additive - nothing here "
