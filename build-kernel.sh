...
# usable, the four ipset hash types stock GKI omits, plus
# NET_ACT_CONNMARK / INET_RAW_DIAG / CIFS_XATTR.
#
# The reason to want nftables specifically: current Debian and Kali ship
# /usr/sbin/iptables as xtables-nft-multi, so iptables commands inside a
# chroot are translated to nftables and fail outright when the kernel
# has no NF_TABLES. Everything in the set is additive - it does not
# change any existing behaviour, and Android's netd keeps using the
# xtables path untouched.
#
...
# and injection in the Kali chroot.
#
# CFG80211/MAC80211 are turned on as =m ONLY, so the driver has
# something to link against. Nothing wireless goes into the Image, and
# the cfg80211.ko/mac80211.ko this produces are deliberately NOT
# packaged: the phone loads Qualcomm's from /vendor/lib/modules, and a
# second copy of that symbol surface is exactly what killed internal
# WiFi the first time this was attempted. Never set these to =y.
#
# The build verifies the modules' symbol CRCs against the vendor stack
